...
//point where mouse resides.

const double PI = 3.1415926535;
const double PI_under_180 = 180.0 / PI;
...
void Sprite_Transform_Draw_Pivot(LPDIRECT3DTEXTURE9 image, int x, int y, int width, int height, int pivotX, int pivotY,
    float rotation, float scaling, D3DCOLOR color)
{
    ...
    //set center by dividing width and height by two
    D3DXVECTOR2 center( (float)pivotX * scaling, (float)pivotY * scaling );
...
int Collision(RECT &dest, RECT rect1, RECT rect2)
{
...
void getMouse(int &x, int &y, HWND hWnd)
{
	POINT pt;
...
void getMouseCoordinates(int &x, int &y, HWND hwnd) //in the screen.
{
	/*POINT windowCoordinate;
	POINT cursorCoordinate;

	if (GetCursorPos(&cursorCoordinate) != false)
	{
		if (ScreenToClient(hwnd, ) != false)
		{
			x = cursorCoordinate.x; - windowCoordinate.x;
			y = cursorCoordinate.y; - windowCoordinate.y;
		}
...
void FontPrint(LPD3DXFONT font, int xf, int yf, string text, D3DCOLOR color)
{
    //figure out the text boundary
    RECT rect1 = { xf, yf, 0, 0 };
    font->DrawText( NULL, text.c_str(), text.length(), &rect1, DT_CALCRECT, color); 

    //print the text
    font->DrawText(spriteobj, text.c_str(), text.length(), &rect1, DT_LEFT, color); 
}
